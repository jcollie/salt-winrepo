nxlog:
  2.6.1131:
    installer: 'http://downloads.sourceforge.net/project/nxlog-ce/nxlog-ce-2.6.1131.msi'
    full_name:  'NXLOG-CE'
    reboot: False
    install_flags: '/quiet /norestart'
    msiexec: True
    uninstaller: 'http://downloads.sourceforge.net/project/nxlog-ce/nxlog-ce-2.6.1131.msi'
    uninstall_flags: '/quiet /norestart'
  2.5.1089:
    installer: 'http://downloads.sourceforge.net/project/nxlog-ce/nxlog-ce-2.5.1089.msi'
    full_name:  'NXLOG-CE'
    reboot: False
    install_flags: '/quiet /norestart'
    msiexec: True
    uninstaller: 'http://downloads.sourceforge.net/project/nxlog-ce/nxlog-ce-2.5.1089.msi'
    uninstall_flags: '/quiet /norestart'
